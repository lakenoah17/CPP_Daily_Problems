...
// subdir1 and a sub-directory subdir2 containing a file
// file.ext.
//
//The string "dir\n\tsubdir1\n\t\tfile1.ext\n\t\tsubsubdir1
...
// second-level sub-directory subsubdir2 containing a file
// file2.ext.
//
...
int main(){
    std::string directoryStructure = "dir\n"
                               "    subdir1\n"
                               "        file1.ext\n"
                               "        subsubdir1\n"
                               "    subdir2\n"
...
int findLongestPathName(std::string directoryStructure, int currLongest){
    int finalIndex = directoryStructure.find('\n');
    std::string name = directoryStructure.substr(0, finalIndex);
    int begginingIndex = name.find_last_of(' ');

    if (begginingIndex == -1){
        return findLongestPathName(directoryStructure.substr(finalIndex), name.size());
    }
